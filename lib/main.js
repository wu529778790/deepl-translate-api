 ...
 * @param {number} options.delay - 每次翻译之间的延迟时间(毫秒)，默认2000ms
 * @param {boolean} options.continueOnError - 遇到错误时是否继续翻译其他文本，默认true
...
async function translateBatch(texts, targetLang = "zh", options = {}) {
  try {
    ...
    console.log(`⏱️ 翻译间隔: ${delay}ms`);
    console.log(`🔄 遇错继续: ${continueOnError ? "是" : "否"}`);
    ...
    for (let i = 0; i < texts.length; i++) {
      ...
      console.log(
        `\n[${i + 1}/${texts.length}] 正在翻译: "${currentText.substring(
          0,
          50
      ...
      try {
        ...
        if (onProgress && typeof onProgress === "function") {
          try {
            onProgress(i + 1, texts.length, results[results.length - 1]);
          } catch (callbackError) {
        ...
        console.error(
          `❌ [${i + 1}/${texts.length}] 翻译失败: ${error.message}`
        );

        // 调用进度回调（即使失败）
        if (onProgress && typeof onProgress === "function") {
          try {
            onProgress(i + 1, texts.length, errorResult);
          } catch (callbackError) {
            console.warn(`⚠️ 进度回调函数执行失败:`, callbackError.message);
          }
        }
