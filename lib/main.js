  ...
  try {
    ...
    if (targetLang !== "zh" && targetLang !== "zh-cn") {
      const targetLangName =
        LANGUAGE_MAP[targetLang.toLowerCase()] || targetLang;
      try {
        // 点击目标语言区域
        ...
        // 查找并点击对应的目标语言选项
        const targetLangButton = await page.evaluate((langName) => {
          const buttons = Array.from(document.querySelectorAll("button"));
          const targetButton = buttons.find(
            (btn) => btn.textContent && btn.textContent.includes(langName)
          );
          if (targetButton) {
            targetButton.click();
            return true;
        ...
        if (targetLangButton) {
          await sleep(500);
          console.log(`✅ 已设置目标语言: ${targetLangName}`);
        }
      } catch (error) {
    ...
    if (sourceLang !== "auto") {
      const sourceLangName =
        LANGUAGE_MAP[sourceLang.toLowerCase()] || sourceLang;
      try {
        // 点击源语言选择按钮
        ...
        // 查找并点击对应的源语言选项
        const sourceLangButton = await page.evaluate((langName) => {
          const buttons = Array.from(document.querySelectorAll("button"));
          const targetButton = buttons.find(
            (btn) => btn.textContent && btn.textContent.includes(langName)
          );
        ...
        if (sourceLangButton) {
          await sleep(500);
          console.log(`✅ 已设置源语言: ${sourceLangName}`);
        }
      } catch (error) {
...
async function getSession() {
  try {
    ...
    console.log("✅ 浏览器会话已就绪");
    return "webpage-managed";
  } catch (err) {
